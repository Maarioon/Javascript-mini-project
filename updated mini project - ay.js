...
function extractTeamNamesAndScores(strMatchResult){
    let objMatchResult = {};
    let arrTeamAndScore = strMatchResult.split('-');
    let arrTeam1 = arrTeamAndScore[0].trim().split(' ');
    let arrTeam2 = arrTeamAndScore[1].trim().split(' ');

    let team1Score = arrTeam1.pop();
    let team2Score = arrTeam2.shift();

    let team1Name = arrTeam1.join(' ');
    let team2Name = arrTeam2.join(' ');

    objMatchResult[team1Name] = team1Score;
    objMatchResult[team2Name] = team2Score;
...
// Call the function with the match result string
let result = extractTeamNamesAndScores(strSampleResult);

// Log the result
for (const [team, score] of Object.entries(result)) {
